...
- 진행 중인 작업: 임시 EC2 네트워크 테스트 종료 후 drift 정리와 인벤토리 누락 범위 확인
- 아직 수행하지 않은 작업: 기본 네트워크·자동 생성 자원·백업 및 배포 자동화의 Terraform 소유 경계 확정
...
1. 팀원의 임시 EC2 네트워크 테스트 종료 후 보안 그룹 drift 정리와 project plan `No changes` 복구
2. 기본 VPC·서브넷, 자동 생성 Glue 자원과 나머지 IAM 주체의 Terraform 소유 경계 확정
3. 백업·스냅샷·배포 자동화와 연결된 AWS 자원 조사
4. 2026-10-17 파이프라인 백업 AMI·스냅샷의 삭제·연장 여부 검토
5. 정기 drift 탐지와 실제 서비스 데이터 복구 시험을 운영 고도화 작업으로 진행
