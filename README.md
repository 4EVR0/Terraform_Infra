...
1. 모니터링 서버에 SSM 관리 경로 추가 및 실제 명령 검증
2. 검증 성공 후 모니터링의 공용 SSH 그룹 분리
3. 파이프라인의 대체 관리 접속 경로와 장기 보존 여부 확정
4. 팀원별 상태 접근 검증과 최소 권한 로그인 방식 확정
5. 버전 관리·보존 정책을 별도 운영 변경으로 설계
6. 인벤토리 누락 범위 조사 및 공유 자원 경계 확정
7. 팀 접근 권한 구성 및 팀원별 상태 접근 검증
