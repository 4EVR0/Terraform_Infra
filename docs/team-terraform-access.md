...
- `4EVR0TeamAdminRole`, `4EVR0TeamUsers`와 세 사용자 멤버십 확인
- 역할 신뢰 대상을 등록된 세 사용자로 제한하고 MFA 필수 조건 확인
- 역할 최대 세션 2시간, `AdministratorAccess`와 보호 정책 연결 확인
- 팀 사용자 기본 정책이 비밀번호·MFA 관리와 역할 전환만 허용하는 상태 확인
- 적용 후 비상 관리자 역할 세션의 bootstrap plan에서 **No changes** 확인
- 팀원별 최초 비밀번호 변경·MFA 등록·역할 전환은 사용 절차와 함께 각 팀원에게 인계. 개인별 완료 여부는 이 저장소에서 별도로 검증하지 않음
