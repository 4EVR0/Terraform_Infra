...
- EC2 user data 보안 검사: **통과**
- 기존 관리 자원 변경: 없음
- 실제 import 적용: 미수행

최초 계획은 user data 보안 검사에서 차단하고 폐기했다. 보안 조치 후 만든 새 계획은 GraphDB EC2 import 1건만 포함하며 저장된 계획과 JSON은 Git 제외 로컬 경로에서 관리한다.

## 보안 조치 실행 결과 — 2026-09-12

- 노출된 Neo4j 비밀번호를 새 임의 값으로 교체
- GraphDB 서버 `.env`, 로컬 추천 서버 `.env`, GitHub Actions secret을 같은 새 값으로 갱신
- 서버 `.env` 권한을 소유자 전용 `600`으로 제한
- 공개 저장소와 운영 서버의 적재 완료 로그에서 비밀번호 출력 제거
- SSH 비밀번호·키보드 대화형 로그인 비활성화 및 공개키 로그인 유지
- 사용자 홈에 남아 있던 개인키 두 개를 root 전용 디렉터리로 격리
- 공개키가 있는 팀 계정의 기존 로컬 비밀번호 잠금
- Neo4j 정상 종료와 EC2 중지 후 루트 EBS 복구 스냅샷 생성
- EC2 user data를 빈 값으로 변경한 뒤 인스턴스 재시작
- Neo4j와 Promtail 재기동, 관리자 공개키 SSH 접속과 추천 서버의 실제 그래프 조회 확인
- 재시작 전후 그래프 노드·관계 수 일치 확인

복구 스냅샷은 기존 암호화되지 않은 루트 볼륨을 따라 암호화되지 않은 상태다. 암호화된 볼륨으로의 교체는 최초 무변경 import와 분리한 운영 개선으로 진행한다. 팀원별 사용자 계정의 공개키 접속은 각자의 장비에서 추가 확인이 필요하다.
...
4. EC2 중지 후 복구 스냅샷 생성과 기존 user data 삭제
...
위 기준은 이번 작업에서 모두 수행했으며, 아래 내용은 같은 문제가 발생했을 때 사용할 실행 절차다.
...
- 팀원별 공개키 접속 확인과 암호화되지 않은 기존 루트 볼륨 개선은 후속 작업
