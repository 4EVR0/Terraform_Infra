...
- 파이프라인 테스트 EC2의 현재 사용 여부·소유 범위·종속 자원 조사
- 조사 결과에 따라 편입 또는 폐기 후보로 분류

## 2026-09-12 — 파이프라인 테스트 EC2 편입 가치 조사
...
- 테스트 용도로 생성된 뒤 장기간 중지 상태
- 프로젝트 문서와 코드에서 해당 EC2를 현재 운영 경로로 참조하는 근거 없음
...
- 장기 관리 대상이라는 근거가 부족하므로 Terraform 편입 보류
- 서버 내부에 문서화되지 않은 코드·설정·데이터가 있을 가능성은 읽기 전용 외부 조회만으로 배제할 수 없음
- 담당자에게 재사용 계획, 보존 데이터, Airflow 서버로의 역할 대체 여부, 유지 책임 확인 필요
- 확인 결과가 계속 사용이면 EC2 import 준비, 사용 종료이면 백업 필요성 검토 후 별도 정리 계획 수립
- 이번 조사에서는 AWS 자원을 생성·수정·시작·삭제하지 않음

세부 판단 기준은 [파이프라인 테스트 EC2 관리 판단](assess-pipeline-ec2.md) 참조.
