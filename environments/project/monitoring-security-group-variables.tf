variable "monitoring_security_group" {
  description = "Reviewed group and existing rule IDs; actual values belong in ignored local tfvars. Keep logical rule keys stable."
  type = object({
    id          = string
    name        = string
    description = string
  ...
  validation {
    condition = alltrue([
      for rule in concat(values(var.monitoring_security_group.ingress), values(var.monitoring_security_group.egress)) :
